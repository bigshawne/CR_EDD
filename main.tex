...
\subsection{Result of the Current Agent}
...
\begin{figure}[H]
    \caption{Training 700000 timesteps}
    \centering
...
\section{Future Work}
In the upcoming work, we will focus on the following aspects:

\begin{itemize}
    \item We will keep training our agent on PPO and try tune hyper parameters since as the current result on Figure 16 shows that we keep getting better and better score.
    \item Game state design. A large proportion of error comes from image processing and object detection. Judging from the results, it seems that our agent still performs badly when the game scene is simple. We think that we should improve the way we transform game play scene into states and change the structure of CNN.
    \item Training efficiency. Due to the restrictions of our training equipment, the training process of our agent is slow and the training of our models are still limited to 30000 frames. We might try other training methods or apply Virtual Machine and Cloud Server to speed up our training.
    \item We'll be keep on trying other methods like transfer learning and genetic algorithm. Recent research shows that training on a easier game and then apply it on the target game will be beneficial. So we’ll try to apply AI agent that has already been trained on similar games like \textit{Frogger}, and then train it on \textit{Crossy Road}. Also genetic algorithm is widely used in implementing game AI, applying it with our agent might give a boost in performance.
\end{itemize}
